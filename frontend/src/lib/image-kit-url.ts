 ...
 */
function buildNorthwindTextLayer({
  w,
  h,
}: {
  w: number | null;
 ...
 * @returns {boolean}
 */
function isImageKitDeliveryUrl(url: string) {
  try {
    const u = new URL(url);
    if (u.hostname.endsWith("ik.imagekit.io")) return true;
    const endpoint = process.env.VITE_IMAGEKIT_URL_ENDPOINT?.replace(/\/$/, "");
 ...
 */
export function imageKitWatermarkedUrl(url: string, opts = {}) {
